...
class MapToOdomPathController(Node):
    ...
    def __init__(self):
        ...
        # -----------------------------
        self.exploration_active = False   # safer default
        self.prev_exploration_active = False
        self.v_max = 0.28
        self.w_max = 0.68
        self.v_th=0.27
        self.v_min = 0.28
        ...
        self.rotate_threshold = math.radians(10)
        ...
        # Subscriptions
        # -----------------------------
        self.create_subscription(Bool, '/exploration', self.exploration_cb, 10)
        ...
        self.goal_yaw = None
        self.rotate_at_goal = True   # default behavior
        # Pheromone map
        self.pher_map = None
        ...
        self.goal_abort_pub = self.create_publisher(Bool, '/goal_abort', 10)
        ...
        self.backward_duration = 0.5  # seconds (tune this)

        self.backward_msg = Twist()
        self.backward_msg.linear.x = -0.15
        self.backward_msg.angular.z = 0.0
        # -----------------------------
        # Timer
        # -----------------------------
        self.create_timer(0.05, self.control_loop)
        self.get_logger().info(" Controller Ready")
    def exploration_cb(self, msg: Bool):
        self.prev_exploration_active = self.exploration_active
        self.exploration_active = msg.data
    def goal_callback(self, msg: PoseStamped):
        # Extract yaw
        q = msg.pose.orientation
        self.goal_yaw = self.quaternion_to_yaw(q)

        # 🔥 Control rotation behavior using orientation.x
        if msg.pose.orientation.x == 1.0:
            self.rotate_at_goal = False
            #self.get_logger().info("Goal received → NO rotation at goal")
        else:
            self.rotate_at_goal = True
            #self.get_logger().info("Goal received → WILL rotate at goal")
    # -----------------------------
    # Callbacks
    # -----------------------------
    ...
    def control_loop(self):
        ...
        if self.prev_exploration_active and not self.exploration_active:
            self.get_logger().warn("🛑 Exploration stopped → stopping robot")

            # self.stop_robot()
            # self.goal_abort_pub.publish(Bool(data=True))

            # # Optional: clear path so it doesn't resume
            # self.path = []
        ...
        if dist_to_goal < self.goal_tolerance:
            ...
            #  Case 1: Do NOT rotate
            if not self.rotate_at_goal:
                self.stop_robot()
                self.get_logger().info("🧸 Goal reached (no rotation)")
                self.goal_reached_pub.publish(Bool(data=True))
                self.path = []
                return

            #  Case 2: Rotate to match goal orientation
            if self.goal_yaw is not None:
                yaw_error = self.normalize_angle(self.goal_yaw - yaw)

                if abs(yaw_error) > self.rotate_threshold:
                    cmd = Twist()
                    cmd.linear.x = 0.0
                    cmd.angular.z = math.copysign(
                        min(self.w_max, self.k2 * abs(yaw_error)),
                        yaw_error
                    )

                    self.get_logger().info(f"🔄 Rotating to goal yaw | error: {yaw_error:.3f}")
                    self.cmd_pub.publish(cmd)
                    return

            # Done
            self.stop_robot()
            self.get_logger().info("🧸 Goal reached (with rotation)")
            self.goal_reached_pub.publish(Bool(data=True))
            self.path = []
            return
        # if dist_to_goal < self.goal_tolerance:
        ...
        if path_blocked:
            ...
            # START backward if needed
            if path_blocked and not self.backward_active:
                self.get_logger().warn("🚨 Path blocked → starting backward motion")

                self.backward_active = True
                self.backward_end_time = now + self.backward_duration
