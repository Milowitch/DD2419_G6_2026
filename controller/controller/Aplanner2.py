...
class SimpleAStarPlanner(Node):
    def __init__(self):
        ...
        self.declare_parameter("b_spline_smooth", 0.005)
        ...
        self.last_goal_world = None  # store last goal to check duplicates
    ...
    def goal_callback(self, msg):
        ...
        # ignore if goal is the same as previous
        if self.last_goal_world is not None:
            lx, ly, lyaw = self.last_goal_world
            if math.isclose(x, lx, abs_tol=1e-3) and math.isclose(y, ly, abs_tol=1e-3) and math.isclose(yaw, lyaw, abs_tol=1e-3):
                self.get_logger().info("Duplicate goal received, ignoring")
                return
    ...
    def goal_reached_cb(self, msg):
        ...
        self.goal = None
        self.goal_world = None
        self.last_goal_world = None  # allow next goal
    ...
    def smooth_path_bspline(self, path):
        ...
        try:
            ...
            thetas = [math.atan2(ys_smooth[i+1]-ys_smooth[i], xs_smooth[i+1]-xs_smooth[i]) for i in range(len(xs_smooth)-1)]
